 ...
 */
export type Reaction = 'surprised' | 'happy' | 'wink' | 'dizzy'
...
export class FaceMotion {
  ...
  grabbing = false
  reaction: { kind: Reaction; started: number } | null = null
  targetX = 0
  targetY = 0
  blinkAt = 4.2

  reset(time = this.time) {
    this.time = time
    this.grabbing = false
    this.reaction = null
    this.targetX = this.targetY = 0
    this.blinkAt = this.time + 4.2
    for (const key of Object.keys(this.state) as (keyof FacePose)[]) this.state[key] = 0
  }
  ...
  grab(active: boolean, celebrate = true) {
  ...
  update(time: number) {
    ...
    let kind = this.grabbing ? 'squish' : 'idle',
      weight = this.grabbing ? 1 : 0
    if (this.reaction) {
    ...
    for (const key of ['surprised', 'squish', 'happy', 'wink', 'dizzy'] as const) {
    ...
    this.state.blink =
      !this.reducedMotion && kind === 'idle' && blinkTime > 0 && blinkTime < 1
        ? Math.sin(blinkTime * Math.PI) ** 2
        : 0
    this.state.gazeX +=
      ((this.reducedMotion ? 0 : this.targetX) - this.state.gazeX) * (1 - Math.exp(-dt * 10))
    this.state.gazeY +=
      ((this.reducedMotion ? 0 : this.targetY) - this.state.gazeY) * (1 - Math.exp(-dt * 10))
    return this.state
  }
