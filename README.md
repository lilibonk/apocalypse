...
项目执行入口：[OPC 与 Linear 产品项目路由](docs/opc-workflow.md)。所有 Apocalypse 任务归入 Lilibonk Team 下的 Apocalypse Project，阶段以 Milestone 管理。

## 特性

- **前端品牌**：登录舞台使用 Three.js WebGPU 软体史莱姆；无 WebGL 回退，动效关闭/不支持时静态降级。[方案与验收](apocalypse-web/docs/brand-slime/acceptance.md)

- **模块化单体**：包即模块（`common`/`framework`/`system`/`calendar`），Modulith `verify()` 测试期强制边界，跨模块只走 facade 与共享事件契约。Order 早期示例已退役，仅保留历史事件兼容桥及原数据库/种子；不再提供 `/orders` API。
- **原生模块装配**：业务域通过自己的 `@ModuleConfiguration` 接入；Calendar 默认关闭时仅保留能力声明、guard 和稳定 facade，内部组件/Mapper/专属配置不装配。开启 `apocalypse.capabilities.calendar.enabled=true` 并重启后完整装配；共享数据库、Flyway、安全与数据始终保留。新能力由模块声明 key，未知 key 关闭、重复声明拒绝，权限缓存按全部能力状态隔离。
- **前端单一模块归属**：模块自己的 `*.queries.ts` 声明唯一 `ModuleScope`，页面与 DynaLayer 共用原生 queryOptions/操作定义。最新 `/me` 驱动模块及权限代次；撤权、换号、刷新或编辑器切换后，旧查询/操作结果不再回填、弹提示或下载。对象级 403/404 清除受影响资源与编辑态并更新上下文，不清其他模块/核心缓存。接入及禁止旁路约束见 `apocalypse-web/AGENTS.md`。
...
OpenAPI 的 JSON 响应描述与统一包装同步：裸数据、分页和 void 返回均描述为 `R`，已返回 `R` 的认证接口不重复包装。`code != 0` 的业务错误仍使用 HTTP 200、`data=null`，无效 Bearer JWT 的原生 HTTP 401 单独描述。登录、刷新和 client token 入口在文档中不要求预先携带 JWT。

字段描述遵循现有 JSON：装箱 `Long` 输出为十进制字符串，输入兼容字符串和整数；原生 `long`（如分页总数）保持整数。`LocalDateTime` 使用 `yyyy-MM-dd HH:mm:ss`，已核验的可空字段显式声明 nullable。下载声明实际 MIME、附件头和 JSON 错误响应，客户端应同时接受下载 MIME 与 `application/json`。operationId 由 Controller 类和方法名确定，在可选能力启停时保持稳定；已有生成客户端需要重新生成并验证。字符串 ID 转换后的范围和数值约束、字段间业务约束仍由服务端校验。

HTTP 业务结果指标为 `apocalypse_http_business_requests_total`，仅含 `route`（路由模板或 `UNKNOWN`）与 `result` 标签，可区分 HTTP 200 中的业务失败。只有 `R.code=0` 计为 `success`；二进制等无 `R` 的成功响应为 `unclassified`。该指标复用现有 Actuator/Micrometer，生产默认暴露配置保持不变；采集和告警需另行配置。
...
- **登录安全**：连续失败 5 次锁定 10 分钟（Redis 计数）；`/auth/login` 默认限流 20 次/分钟/IP（`apocalypse.ratelimit.limits.login` 可调）；新建/重置与首次 bootstrap 密码均执行强密码策略；管理员强退与主动注销均经 PostgreSQL 凭证代次撤销该用户全部设备的 access/refresh 令牌，包括刷新前的旧 access。强退所选在线条目已经失效时须刷新列表后重试，Redis 清理失败不撤回数据库撤销结果。
- **限流配置切换**：已有 Redis 限流器的阈值/窗口与本机配置不一致时，返回现有系统错误码 `50000`，不会静默沿用旧值或重置额度。调整前统一停止旧实例流量，等待旧/新窗口中的较大值，再按 `SCAN` 清单逐个使用 `scripts/redis/reset-idle-rate-limiter.lua` 清理已空闲的指定限流器，最后以一致配置恢复全部实例；回滚同样处理。脚本核验配置及三键归属并拒绝近期仍有消费的状态；它不证明旧实例已退出，也不支持新旧配置无停机混跑。认证参数仍由运维环境管理。
...
- **Calendar 发布校验升级**：新草稿内容采用无歧义的版本化编码，草稿版本高于全部留存历史（含已丢弃修订）。旧版托管草稿需重新保存、复核后发布；投影草稿由来源更新后复核。部署时须排空旧写入实例，不能在旧实现与新实现间混用写入；未经替代代次方案，不得物理清理用于防止审批复用的修订历史。
